node('EPBYMINW1374') {
	...
	sh "curl -v --user 'admin:admin123' --upload-file ./artifacts/artifact.tar-'$BUILD_NUMBER'.tar.gz http://10.6.102.44/repository/mnt-pipeline/artifact.tar-'$BUILD_NUMBER'.tar.gz"
    }
    stage('Custom') {
	sh "tree"
    }
